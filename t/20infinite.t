...
my $pos = DateTime::Lite::Infinite::Future->new;
my $neg = DateTime::Lite::Infinite::Past->new;
my $posinf = DateTime::Lite::INFINITY;
my $neginf = DateTime::Lite::NEG_INFINITY;
my $nan_string = DateTime::Lite::NAN;
...
# infinite date math
{
    ...
    my $long_ago = DateTime::Lite->new( year => -100_000 );

    ok( $neg < $long_ago,
        'negative infinity is really negative' );

    my $far_future = DateTime::Lite->new( year => 100_000 );
    ok( $pos > $far_future,
...

{
    ...
    is( DateTime::Lite->compare($pos, $now),  1, 'positive infinite is greater than now' );
    is( DateTime::Lite->compare($neg, $now), -1, 'negative infinite is less than now' );
}

{
    my $now = DateTime::Lite->now;
    my $pos2 = $pos + DateTime::Lite::Duration->new( months => 1 );
...
{
    my $now = DateTime::Lite->now;
    my $neg2 = $neg + DateTime::Lite::Duration->new( months => 1 );
