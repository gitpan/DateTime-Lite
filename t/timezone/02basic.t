...
foreach my $name (@names) {
    my $tz = DateTime::Lite::TimeZone->load( name => $name );
    ...
    {
        my $dt;
        eval { $dt = DateTime::Lite->now( time_zone => $name ) };
        is( $@, '', "Can call DateTime::Lite->now with $name" );
        eval { $dt->add( years => 50 ) };
        is( $@, '', "Can add 50 years with $name" );
        eval { $dt->subtract( years => 400 ) };
        is( $@, '', "Can subtract 400 years with $name" );
        eval { $dt = DateTime::Lite->new( year => 2000, month => 6, hour => 1, time_zone => $name ) };
        is( $dt->hour, 1, 'make sure that local time is always respected' );
        eval { $dt = DateTime::Lite->new( year => 2000, month => 12, hour => 1, time_zone => $name ) };
        is( $dt->hour, 1, 'make sure that local time is always respected' );
    }
}
...
my $tz = DateTime::Lite::TimeZone->load( name => 'America/Chicago' );
...
# minimizes how much of DateTime::Lite.pm needs to work for these tests.
{
    my $dt = DateTime::Lite->new( year => 2001,
                            month => 9,
                            day => 10,
                            time_zone => 'UTC',
                          );
...

{
    ...
    my $dt = DateTime::Lite->new( year => 2200,
                            month => 11,
                            day => 2,
                            time_zone => 'UTC',
                          );
...

{
    ...
    # max year
    my $tz = DateTime::Lite::TimeZone->load( name => 'America/Los_Angeles' );

    my $dt = eval { DateTime::Lite->new( year => $tz->max_year + 1,
                                   month => 5,
                                   day => 20,
                                   time_zone => $tz
                                 ) };
...
{
    my $dt = DateTime::Lite->new( year => 1944,
                            month => 10,
                            day => 29,
                            time_zone => 'UTC',
                          );
...
{
    my $dt = DateTime::Lite->new( year => 1936,
                            month => 3,
                            day => 2,
                            time_zone => 'UTC',
                          );
...
{
    my $dt = DateTime::Lite->new( year => 1883,
                            month => 1,
                            day => 29,
                            time_zone => 'UTC',
                          );
...

{
    ...
    my $dt = DateTime::Lite->from_object( object => TestHack->new );
...
{
    my $tz = DateTime::Lite::TimeZone->load( name => '-0100' );
