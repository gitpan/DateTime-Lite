...
use DateTime::Lite qw(Arithmetic Strftime);
...

{
    ...
    my $dt = DateTime::Lite->new( year => 1997, month => 3, day => 29,
                            hour => 15, minute => 59,
                            time_zone => 'UTC' );
...
# same tests without using UTC as intermediate
{
    ...
    my $dt = DateTime::Lite->new( year => 1997, month => 3, day => 30,
                            hour => 1, minute => 59,
...
{
    # one minute before change to standard time
    my $dt = DateTime::Lite->new( year => 2002, month => 10, day => 26,
                            hour => 15, minute => 59,
                            time_zone => 'UTC' );
...
# same tests without using UTC as intermediate
{
    my $dt = DateTime::Lite->new( year => 2002, month => 10, day => 27,
                            hour => 1, minute => 59,
                            time_zone => 'Australia/Sydney' );
...
{
    # Can't start at 1:59 or we get the _2nd_ 1:59 of that day (post-DST change)
    my $dt = DateTime::Lite->new( year => 2040, month => 4, day => 1,
                            hour => 1, minute => 59,
                            time_zone => 'Australia/Sydney' );
...
{
    my $dt = DateTime::Lite->new( year => 2040, month => 10, day => 7,
                            hour => 1, minute => 59,
                            time_zone => 'Australia/Sydney' );
...
{
    # one minute before change to standard time
    my $dt = DateTime::Lite->new( year => 1982, month => 3, day => 28,
                            hour => 0, minute => 59,
                            time_zone => 'UTC' );
...
{
    # one minute before change to standard time
    my $dt = DateTime::Lite->new( year => 1997, month => 10, day => 26,
                            hour => 0, minute => 59,
                            time_zone => 'UTC' );
...
# future
{
    my $dt = DateTime::Lite->new( year => 2040, month => 3, day => 25,
                            hour => 1, minute => 59,
                            time_zone => 'Europe/Vienna' );
...
{
    my $dt = DateTime::Lite->new( year => 2040, month => 10, day => 28,
                            hour => 1, minute => 59,
                            time_zone => 'Europe/Vienna' );
...
{
    my $dt = DateTime::Lite->new( year => 1977, month => 10, day => 20,
                            hour => 23, minute => 59,
...
{
    my $dt = DateTime::Lite->new( year => 2000, month => 10, day => 5,
...

{
    ...
    my $dt = DateTime::Lite->new( year => 2003, month => 10, day => 26,
...

{
    eval
    {
        DateTime::Lite->new( year => 2003, month => 4, day => 6,
                       hour => 2, time_zone => 'America/Chicago',
                     )
    };
...

{
 ...
 SKIP:
    {
        skip "DateTime::Lite 0.29 has a date math bug that causes this test to fail", 1
            if ( DateTime::Lite->VERSION >= 0.29 && DateTime::Lite->VERSION < 0.30 );

        my $dt = DateTime::Lite->new( year => 2003, month => 4, day => 5,
                                hour => 2,
                                time_zone => 'America/Chicago',
                              );
...
{
    my $dt = DateTime::Lite->new( year => 2003, month => 4, day => 6,
                            hour => 3, time_zone => 'America/Chicago',
                          );
...

{
    eval
    {
        DateTime::Lite->new( year => 2040, month => 3, day => 11,
                       hour => 2, minute => 59, second => 59,
                       time_zone => 'America/Chicago',
                     );
    };
...
{
    my $dt =
        DateTime::Lite->new( year => 2001, month => 10, day => 28,
                       hour => 0, minute => 59,
                       time_zone => 'UTC' );
...

{
    ...
    my $local_tz = DateTime::Lite::TimeZone->load( name => 'America/Chicago' );
    my $utc_tz   = DateTime::Lite::TimeZone->load( name => 'UTC' );

    my $dt = DateTime::Lite->new( year => 2050, time_zone => $local_tz );

    my $sixm = DateTime::Lite::Duration->new( months => 6 );
...
{
    my $local_tz = DateTime::Lite::TimeZone->load( name => 'America/New_York' );
    my $utc_tz   = DateTime::Lite::TimeZone->load( name => 'UTC' );

    my $dt = DateTime::Lite->new( year => 2060, time_zone => $local_tz );

    my $neg_sixm = DateTime::Lite::Duration->new( months => -6 );
