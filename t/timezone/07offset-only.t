...
eval { DateTime::Lite::TimeZone::OffsetOnly->new( offset => 'bad' ) };
is( $@, "Invalid offset: bad\n",
    'test that OffsetOnly does not allow invalid offsets' );

my $off = DateTime::Lite::TimeZone::OffsetOnly->new( offset => '-0100' );
...
foreach ( @good_offsets ) {
    my $off = DateTime::Lite::TimeZone::OffsetOnly->new( offset => $_->[0] );
    is( $off->name, $_->[1], "name matches $_->[1]" );
}

foreach ( @bad_offsets ) {
    eval{ DateTime::Lite::TimeZone::OffsetOnly->new( offset => $_ ) };
