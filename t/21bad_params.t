...
{
    eval { DateTime::Lite->new(%$p) };
    like( $@, qr/did not pass/,
          "Parameters outside valid range should fail in call to new()" );

    eval { DateTime::Lite->new( year => 2000 )->set(%$p) };
    like( $@, qr/did not pass/,
          "Parameters outside valid range should fail in call to set()" );
}

{
    eval { DateTime::Lite->last_day_of_month( year => 2000, month => 13 ) };
    like( $@, qr/did not pass/,
          "Parameters outside valid range should fail in call to last_day_of_month()" );

    eval { DateTime::Lite->last_day_of_month( year => 2000, month => 0 ) };
    like( $@, qr/did not pass/,
          "Parameters outside valid range should fail in call to last_day_of_month()" );
}

{
    eval { DateTime::Lite->new( year => 2000, month => 4, day => 31 ) };
    like( $@, qr/valid day of month/i,
          "Day past last day of month should fail" );

    eval { DateTime::Lite->new( year => 2001, month => 2, day => 29 ) };
    like( $@, qr/valid day of month/i,
          "Day past last day of month should fail" );

    eval { DateTime::Lite->new( year => 2000, month => 2, day => 29 ) };
