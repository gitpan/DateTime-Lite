...
for (# min and max supported days (for 32-bit system)
    ...
    is( join('/',DateTime::Lite::Util::rd2ymd($_->[0])), join('/',@{$_}[1..3]),
        $_->[0] . "   \t=> " . join '/', @{$_}[1..3] );
    }

    is( DateTime::Lite::Util::ymd2rd(@{$_}[1..3]), $_->[0],
        join('/',@{$_}[1..3]) . "   \t=> " . $_->[0]);
}
...
while ( $y <= 4800 ) {
    ...
    # test $y,$m,1
    ++$dno;
    $dno2 = DateTime::Lite::Util::ymd2rd( $y, $m, 1 );
    if ( $dno != $dno2 ) {
        is( $dno2, $dno, "greg torture test: _ymd2rd($y,$m,1) should be $dno" );
        last;
    }
    ( $y2, $m2, $d2 ) = DateTime::Lite::Util::rd2ymd($dno);
    ...
    $dno += $mlen - 1;
    $dno2 = DateTime::Lite::Util::ymd2rd( $y, $m, $mlen );
    if ( $dno != $dno2 ) {
        is( $dno2, $dno,
