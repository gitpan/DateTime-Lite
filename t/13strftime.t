...
while (<DATA>)
{
    ...
    {
        $params = $_;
        $dt = eval "DateTime::Lite->new( $params, time_zone => 'UTC' )";
        next;
    }
    elsif (/^(\w+)/)
    {
        ...
        $dt = eval "DateTime::Lite->new( $params, time_zone => 'UTC', locale => '$locale' )";
        next;
    }
...
# context
{
    my $dt = DateTime::Lite->new( year => 1800,
                            month => 1,
                            day => 10,
                            time_zone => 'UTC',
                          );
...
{
    my $dt = DateTime::Lite->new( year => 2003,
                            hour => 0,
                            minute => 0
                          ) ;
...
{
    is( DateTime::Lite->new( year => 2003, month => 1, day => 1 )->strftime('%V'),
	'01', '%V is 01' );
}

{
    my $dt = DateTime::Lite->new( year => 2004, month => 8, day => 16,
