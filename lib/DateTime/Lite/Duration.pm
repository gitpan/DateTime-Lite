...
sub _normalize_nanoseconds
{
    ...
    return if
        ( $self->{nanoseconds} == DateTime::Lite::INFINITY()
          || $self->{nanoseconds} == DateTime::Lite::NEG_INFINITY()
          || $self->{nanoseconds} eq DateTime::Lite::NAN()
        );
...
sub compare
{
    ...
    return
        DateTime::Lite->compare( $dt->clone->add_duration($dur1), $dt->clone->add_duration($dur2) );
}
...
sub _add_overload
{
    ...
    if ( blessed $d2 && $d2->isa( 'DateTime::Lite' ) )
    {
        $d2->add_duration($d1);
        return;
    }

    # will also work if $d1 is a DateTime::Lite.pm object
...
sub _subtract_overload
{
    ...
    Carp::croak( "Cannot subtract a DateTime::Lite object from a DateTime::Lite::Duration object" )
        if blessed $d2 && $d2->isa( 'DateTime::Lite' );
...
{
    Carp::croak( 'DateTime::Lite::Duration does not overload comparison.'
...
DateTime::Lite::Duration - Duration objects for date math

=head1 SYNOPSIS

  use DateTime::Lite::Duration;

  $d = DateTime::Lite::Duration->new( years   => 3,
                                months  => 5,
                                weeks   => 1,
                                days    => 1,
                                hours   => 6,
  ...
  my $base_dt = DateTime::Lite->new( year => 2000 );
  my @sorted =
      sort { DateTime::Lite::Duration->compare( $a, $b, $base_dt ) } @durations;

  # Human-readable accessors, always positive, but use
  # DateTime::Lite::Format::Duration instead
  $d->years;
  $d->months;
  $d->weeks;
  $d->days;
...
objects are used whenever you do date math with DateTime::Lite.pm.

See the L<How Date Math is Done|DateTime::Lite/"How Date Math is Done">
section of the DateTime::Lite.pm documentation for more details.  The short
...
the appropriate subtraction/delta method on a C<DateTime::Lite.pm> object.

=head1 METHODS

Like C<DateTime::Lite> itself, C<DateTime::Lite::Duration> returns the object from
mutator methods in order to make method chaining possible.

C<DateTime::Lite::Duration> has the following methods:
...
after taking away the larger units given, so for example:
  ...
  my $dur = DateTime::Lite::Duration->new( years => 1, months => 15 );
...
Math is Done|DateTime::Lite/"How Date Math is Done"> section of the
DateTime::Lite.pm documentation
...
durations, please take a look a C<DateTime::Lite::Format::Duration>.

=item * delta_months, delta_days, delta_minutes, delta_seconds, delta_nanoseconds

These methods provide the information C<DateTime::Lite.pm> needs for doing
...
=item * DateTime::Lite::Duration->compare( $duration1, $duration2, $base_datetime )
...
C<DateTime::Lite.pm> object and comparing the resulting datetimes.  This is
...
If no base datetime is given, then the result of C<< DateTime::Lite->now >>
...
use the C<DateTime::Lite::Format::Duration> module.
