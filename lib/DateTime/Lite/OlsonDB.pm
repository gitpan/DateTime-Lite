...
$PLUS_ONE_DAY_DUR =  DateTime::Lite::Duration->new( days => 1 );
$MINUS_ONE_DAY_DUR = DateTime::Lite::Duration->new( days => -1 );
...
sub _parse_rule
{
    ...
    push @{ $self->{rules}{$name} },
        DateTime::Lite::OlsonDB::Rule->new( name => $name, %rule );
...
sub zone
{
    ...
    return
        DateTime::Lite::OlsonDB::Zone->new
            ( name => $name,
...
sub parse_day_spec
{
    ...
    if ( $day =~ /^last(\w\w\w)$/ )
    {
        my $dow = $DateTime::Lite::OlsonDB::DAYS{$1};

        my $last_day = DateTime::Lite->last_day_of_month( year  => $year,
                                                    month => $month,
                                                    time_zone => 'floating',
                                                  );

        my $dt =
            DateTime::Lite->new( year   => $year,
                           month  => $month,
                           day    => $last_day->day,
                           time_zone => 'floating',
                         );
    ...
    elsif ( $day =~ /^(\w\w\w)([><])=(\d\d?)$/ )
    {
        ...
        my $dt = DateTime::Lite->new( year   => $year,
                                month  => $month,
                                day    => $3,
                                time_zone => 'floating',
                              );
...
sub utc_datetime_for_time_spec {
    ...
    if ($is_utc)
    {
        $utc = DateTime::Lite->new( year   => $p{year},
                              month  => $p{month},
                              day    => $p{day},
                              hour   => $hour,
                              minute => $minute,
    ...
    {
        my $local = DateTime::Lite->new( year   => $p{year},
                                   month  => $p{month},
                                   day    => $p{day},
                                   hour   => $hour,
                                   minute => $minute,
...
package DateTime::Lite::OlsonDB::Zone;
...
sub expand_observances
{
    ...
    for ( my $x = 0; $x < @{ $self->{observances} }; $x++ )
    {
        ...
        my $obs =
            DateTime::Lite::OlsonDB::Observance->new
                ( %p,
        ...
        if (DateTime::Lite::OlsonDB::DEBUG)
        {
...
package DateTime::Lite::OlsonDB::Observance;
...
sub new
{
    ...
    my $offset_from_utc = DateTime::Lite::Util::offset_as_seconds( $p{gmtoff} );
    my $offset_from_std = DateTime::Lite::Util::offset_as_seconds( $p{offset_from_std} );
    ...
    if ( $p{utc_start_datetime} )
    {
        ...
        $local_start_datetime +=
            DateTime::Lite::Duration->new( seconds => $offset_from_utc + $offset_from_std );
...
sub until
{
    ...
    my $utc =
        DateTime::Lite::OlsonDB::utc_datetime_for_time_spec
...
{
    ( defined $_[0]->{until}[1] ?
      $DateTime::Lite::OlsonDB::MONTHS{ $_[0]->{until}[1] } :
      1
    );
}
...
{
    ( defined $_[0]->{until}[2]
      ? DateTime::Lite::OlsonDB::parse_day_spec
...
sub _first_rule
{
    ...
    for ( my $x = 0; $x < @rule_dates; $x++ )
    {
        ...
        warn " Next rule starts:  ", $next_dt->datetime, "\n"
            if $next_dt && DateTime::Lite::OlsonDB::DEBUG;
...
package DateTime::Lite::OlsonDB::Rule;
...
sub new
{
    ...
    if ($save)
    {
        $p{offset_from_std} = DateTime::Lite::Util::offset_as_seconds($save);
    }
    else
    {
        $p{offset_from_std} = 0;
...
sub month { $DateTime::Lite::OlsonDB::MONTHS{ $_[0]->{in} } }
sub on { $_[0]->{on} }
sub at { $_[0]->{at} }
...
sub utc_start_datetime_for_year
{
    ...
    my $day =
        DateTime::Lite::OlsonDB::parse_day_spec( $self->on, $self->month, $year );
...
package DateTime::Lite::OlsonDB::Change;
...
DateTime::Lite::OlsonDB - An object to represent an Olson time zone database
