...
BEGIN {
    local $@;
    eval {
        require DateTime::Lite::TimeZone::Catalog;
    }
}
use DateTime::Lite::TimeZone::Floating;
use DateTime::Lite::TimeZone::Local;
use DateTime::Lite::TimeZone::OffsetOnly;
use DateTime::Lite::TimeZone::UTC;
use DateTime::Lite::OlsonDB;
...
sub load {
    ...
    my $zone;
    if (defined $name) {
        my $links = \%DateTime::Lite::TimeZone::Catalog::LINKS;
    ...
    if (defined $name) {
        ...
        unless ( $name =~ m,/, || $SpecialName{ $name }) {
            if ( $name eq 'floating' ) {
                return $CachedTimeZones{$name} = DateTime::Lite::TimeZone::Floating->new;
            }
            if ( $name eq 'local' ) {
                return $CachedTimeZones{$name} = DateTime::Lite::TimeZone::Local->TimeZone();
            }
    
            if ( $name eq 'UTC' || $name eq 'Z' ) {
                return $CachedTimeZones{$name} = DateTime::Lite::TimeZone::UTC->new;
            }

            return $CachedTimeZones{$name} = DateTime::Lite::TimeZone::OffsetOnly->new( offset => $name );
        }
        $conf = _load_time_zone($name);
    }

    if (! $conf) {
        my $x = $ENV{DATETIME_LITE_DEBUG} ? \&Carp::confess : \&Carp::croak ;
    ...
    if ( $zone->is_olson() ) {
        my $object_version =
            ...
            ? $zone->olson_version()
            : 'unknown';
        my $catalog_version = DateTime::Lite::TimeZone::Catalog->OlsonVersion();

        if ( $object_version ne $catalog_version )
        {
            warn "Loaded $name, which is from an older version ($object_version) of the Olson database than this installation of DateTime::Lite::TimeZone ($catalog_version).\n";
        }
    }
...
sub _load_time_zone {
    ...
    return do "DateTime/Lite/TimeZone/$file";
}
...
sub _generate_spans_until_match
{
    ...
    foreach my $year ( $self->max_year .. $generate_until_year )
    {
        for ( my $x = 0; $x < @rules; $x++ )
        {
            ...
            push @changes,
                DateTime::Lite::OlsonDB::Change->new
                    ( type => 'rule',
                      utc_start_datetime   => $next,
                      local_start_datetime =>
                      $next +
                      DateTime::Lite::Duration->new
    ...
    for ( my $x = 1; $x < @sorted; $x++ )
    {
        ...
        my $span =
            DateTime::Lite::OlsonDB::Change::two_changes_as_span
...
sub is_valid_name
{
    ...
    return $tz && $tz->isa('DateTime::Lite::TimeZone') ? 1 : 0
}
...
sub offset_as_seconds {
    {
        local $@;
        shift if eval { $_[0]->isa('DateTime::Lite::TimeZone') };
    }
    DateTime::Lite::Util::offset_as_seconds(@_);
}
...
sub offset_as_string {
    ...
    DateTime::Lite::Util::offset_as_string(@_);
}

# These methods all operate on data contained in the DateTime/TimeZone/Catalog.pm file.

sub all_names {
    if (! $DateTime::Lite::TimeZone::Catalog::LOADED_CATALOG ) {
        DateTime::Lite::TimeZone::Catalog::load();
    }

    return wantarray ? @DateTime::Lite::TimeZone::Catalog::ALL : [@DateTime::Lite::TimeZone::Catalog::ALL];
}
...
sub categories {
    ...
    return wantarray
        ? @DateTime::Lite::TimeZone::Catalog::CATEGORY_NAMES
        : [@DateTime::Lite::TimeZone::Catalog::CATEGORY_NAMES];
}
...
{
    return
        wantarray ? %DateTime::Lite::TimeZone::Catalog::LINKS : {%DateTime::Lite::TimeZone::Catalog::LINKS};
}

sub names_in_category
{
    shift if $_[0]->isa('DateTime::Lite::TimeZone');
    return unless exists $DateTime::Lite::TimeZone::Catalog::CATEGORIES{ $_[0] };

    return
        wantarray
        ? @{ $DateTime::Lite::TimeZone::Catalog::CATEGORIES{ $_[0] } }
        : [ $DateTime::Lite::TimeZone::Catalog::CATEGORIES{ $_[0] } ];
}
...
{
    wantarray
        ? ( sort keys %DateTime::Lite::TimeZone::Catalog::ZONES_BY_COUNTRY )
        : [ sort keys %DateTime::Lite::TimeZone::Catalog::ZONES_BY_COUNTRY ];
}
...
sub names_in_country
{
    ...
    return unless exists $DateTime::Lite::TimeZone::Catalog::ZONES_BY_COUNTRY{ lc $_[0] };

    return
        wantarray
        ? @{ $DateTime::Lite::TimeZone::Catalog::ZONES_BY_COUNTRY{ lc $_[0] } }
        : $DateTime::Lite::TimeZone::Catalog::ZONES_BY_COUNTRY{ lc $_[0] };
}
...
DateTime::Lite::TimeZone - Time zone object base class and factory
...
=head1 SYNOPSIS
  ...
  my $tz = DateTime::Lite::TimeZone->new( name => 'America/Chicago' );
...
most users will not need to directly use C<DateTime::Lite::TimeZone>
methods.
...
=head2 DateTime::Lite::TimeZone->new( name => $tz_name )
...
DateTime::Lite::TimeZone::America::Chicago class.
...
version of the current DateTime::Lite::TimeZone installation. If they do not
...
C<DateTime::Lite::TimeZone::Floating> object is returned.  A floating time
...
If the "name" parameter is "UTC", then a C<DateTime::Lite::TimeZone::UTC>
object is returned.

If the "name" is an offset string, it is converted to a number, and a
C<DateTime::Lite::TimeZone::OffsetOnly> object is returned.
...
=item * L<DateTime::Lite::TimeZone::Local::Unix>

=item * L<DateTime::Lite::TimeZone::Local::Win32>

=item * L<DateTime::Lite::TimeZone::Local::VMS>
...
=head2 DateTime::Lite::TimeZone->is_valid_name($name)
...
C<DateTime::Lite::TimeZone::Alias>, any aliases you've created will be valid.

=head2 DateTime::Lite::TimeZone->all_names
...
=head2 DateTime::Lite::TimeZone->categories
...
=head2 DateTime::Lite::TimeZone->links
...
=head2 DateTime::Lite::TimeZone->names_in_category( $category )
...
=head2 DateTime::Lite::TimeZone->countries()
...
=head2 DateTime::Lite::TimeZone->names_in_country( $country_code )
...
=head2 DateTime::Lite::TimeZone->offset_as_seconds( $offset )
...
=head2 DateTime::Lite::TimeZone->offset_as_string( $offset )
...
If you subclass C<DateTime::Lite::TimeZone>, you will inherit its hooks,
...
The tools directory of the DateTime::Lite::TimeZone distribution includes
