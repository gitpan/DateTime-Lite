...
use DateTime::Lite::LeapSecond;
use DateTime::Lite::Locale;
use DateTime::Lite::Util;
...
sub import {
    ...
    foreach my $component (@_) {
        eval "require DateTime::Lite::$component";
        die "DateTime::Lite failed to load $component component: $@" if $@;
    }
}
...
sub jd
{
    ...
    my $day_length = DateTime::Lite::LeapSecond::day_length( $self->{utc_rd_days} );
...
sub set_time_zone {
    ...
    $self->{tz} = ref $tz ? $tz : DateTime::Lite::TimeZone->load( name => $tz );
...
sub new {
        ...
        while (my ($key, $spec) = each %spec) {
            ...
            if (my $range = $spec->{range}) {
                ...
                if ( (defined $range->[0] && $v < $range->[0]) ||
                    ...
                    Carp::croak(qq|The '$key' parameter ("$p{$key}") to DateTime::Lite::new did not pass the range test|); # hmm, almost
                }
            }
        }
    }
    ...
    if ($day > DateTime::Lite::Util::month_length($year, $month)) {
    ...
    $self->{locale} = blessed $locale ?
        $locale : DateTime::Lite::Locale->load($locale);

    $self->{tz} = blessed $time_zone ?
        $time_zone : DateTime::Lite::TimeZone->load(name => $time_zone);
    $self->{local_rd_days} = DateTime::Lite::Util::ymd2rd($year, $month, $day);
    $self->{local_rd_secs} = DateTime::Lite::Util::time_as_seconds($hour, $minute, $second);
    $self->{offfset_modifier} = 0;
    $self->{rd_nanosecs} = $nanosecond;
    $self->{formatter} = $p{formatter};

    DateTime::Lite::Util::normalize_nanoseconds($self->{local_rd_secs}, $self->{rd_nanosecs});
...
sub _calc_utc_rd {
    ...
    # except date math.
    DateTime::Lite::Util::normalize_tai_seconds( $self->{utc_rd_days}, $self->{utc_rd_secs} );
}
       ...
       )
    {
        if ( $second < 60 && $offset > 0 )
        {
            $self->{offset_modifier} =
                DateTime::Lite::LeapSecond::day_length( $utc_rd_days - 1 ) - SECONDS_PER_DAY;
        ...
        {
            my $mod = DateTime::Lite::LeapSecond::day_length( $utc_rd_days - 1 ) - SECONDS_PER_DAY;

            unless ( $mod == 0 )
            {
                $self->{utc_rd_secs} -= $mod;

                DateTime::Lite::Util::normalize_seconds($self);
            }
        }
    }
    elsif ( $offset < 0
...
sub _calc_local_rd
{
    ...
    # loops between DateTime.pm and DateTime::Lite::TimeZone
    ...
    {        my $offset = $self->offset;
        ...
        DateTime::Lite::Util::normalize_tai_seconds( $self->{local_rd_days}, $self->{local_rd_secs} );
...
sub _calc_local_components
{
    ...
    @{ $self->{local_c} }{ qw( year month day day_of_week
        ...
        DateTime::Lite::Util::rd2ymd( $self->{local_rd_days}, 1 );

    @{ $self->{local_c} }{ qw( hour minute second ) } =
        DateTime::Lite::Util::seconds_as_components
...
sub from_object {
    ...
    if ( $object->can('time_zone') && ! $object->time_zone->is_floating
         && $rd_secs > 86399 && $rd_secs <= DateTime::Lite::LeapSecond::day_length($rd_days) )
    {
    ...
    my %args;
    @args{ qw( year month day ) } = DateTime::Lite::Util::rd2ymd($rd_days);
    @args{ qw( hour minute second ) } =
        DateTime::Lite::Util::seconds_as_components($rd_secs);
...
sub last_day_of_month {
    ...
    if ($p{month} > 12 || $p{month} < 1) {
        Carp::croak(qq|The 'month' parameter ("$p{month}") to DateTime::Lite::last_day_of_month did not pass the 'is between 1 and 12' callback|);
    }

    return $class->new(%p, day => DateTime::Lite::Util::month_length($p{year}, $p{month}));
}
...
sub week
{
    ...
    unless ( defined $self->{local_c}{week_year} )
    {
        ...
        my $jan_one_dow_m1 =
            ( ( DateTime::Lite::Util::ymd2rd( $self->year, 1, 1 ) + 6 ) % 7 );
...
sub _weeks_in_year
{
    ...
    my $jan_one_dow =
        ( ( DateTime::Lite::Util::ymd2rd( $year, 1, 1 ) + 6 ) % 7 ) + 1;
    my $dec_31_dow =
        ( ( DateTime::Lite::Util::ymd2rd( $year, 12, 31 ) + 6 ) % 7 ) + 1;
...
sub DateTime::Lite::time { goto &hms }

sub iso8601 { join 'T', $_[0]->ymd('-'), $_[0]->hms(':') }
sub datetime { goto &iso8601 }

sub is_leap_year { DateTime::Lite::Util::is_leap_year( $_[0]->year ) }
...
sub leap_seconds
{
    ...
    return DateTime::Lite::LeapSecond::leap_seconds( $self->{utc_rd_days} );
}
...
sub _calc_utc_components
{
    ...
    @{ $self->{utc_c} }{ qw( year month day ) } =
        DateTime::Lite::Util::rd2ymd( $self->{utc_rd_days} );

    @{ $self->{utc_c} }{ qw( hour minute second ) } =
        DateTime::Lite::Util::seconds_as_components( $self->{utc_rd_secs} );
}
...
sub _compare
{
        ...
        (blessed $dt2 && $dt2->can( 'utc_rd_values' ) ))
    {
        ...
        Carp::croak( "A DateTime::Lite object can only be compared to"
                     . " another DateTime::Lite object ($dt1_string, $dt2_string)." );
    }
...
sub from_day_of_year
{
    ...
    my $is_leap_year = DateTime::Lite::Util::is_leap_year( $p{year} );
    ...
    while ( $month <= 12 && $day > DateTime::Lite::Util::month_length( $p{year}, $month ) )
    {
        $day -= DateTime::Lite::Util::month_length( $p{year}, $month );
        $month++;
    }
...
DateTime::Lite - A Low Calorie DateTime
...
=head1 SYNOPSIS
    ...
    # Strftime doesn't come with DateTime::Lite by default
...
If we can simply copy the DateTime::Lite files over via FTP instead of 'make install', that's even better.
...
DateTime::Lite does not intend to be a drop-in replacement for DateTime.pm. 
...
As stated above, DateTime::Lite tries to be as compatible as possible with DateTime.pm, but there are a few places it deliberately changed from DateTime.pm. Some notable differences from DateTime.pm are as follows
...
DateTime::Lite::TimeZone and DateTime::Lite::Locale have big changes from their original counterparts.

First, you do NOT call new() on these objects (unless this is something you explicitly want to do). Instead, you need to call load(). So if you were mucking with DateTime::Lite::TimeZone and DateTime::Locale, you need to find out every occurance of
...
With this version, the objects are mostly the just regular objects, and the exact definition for each timezone/locale is stored in data files. (TODO: They can be located anywhere DateTime::Lite can find them)
...
=item Remove dependency to Catalogs for TimeZone and Locale

You should not need to load the entire catalog just to grab a date
